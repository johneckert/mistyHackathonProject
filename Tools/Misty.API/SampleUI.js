$(document).ready(function () {
	...
	//Reflects the robot model (e.g. Misty 1, Misty 2)
	let robotVersion;
	...
	function populateAudioList() {
		...
		for (var i = 0; i < audioFileList.length; i++) {
			$('#audio-asset-id').append($('<option/>', {
				value: audioFileList[i],
				text: audioFileList[i]
			}));
		}
	}
	...
	function validateImageFile(file) {
		...
		if (!allowedExtensions.exec(file.name)) {
			alert("Please upload file with .jpeg/.jpg/.png/.gif extensions only.");
			return false;
		}
		if (file.size > 3000000) {
			showToastMessage("The file exceeds the maximum size of 3MB.");
			return false;
		}
		return true;
	}
	...
	function validateAudioFile(file) {
		var allowedExtensions = /(\.wav|\.mp3|\.wma|\.aac)$/i;
		if (!allowedExtensions.exec(file.name)) {
			alert("Please upload file with .wav/.mp3/.wma/.aac extensions only.");
			return false;
		}
		if (file.size > 3000000) {
			showToastMessage("The file exceeds the maximum size of 3MB.");
	...
	$("#move-head-form").submit(function (e) {
		e.preventDefault();
		if (robo === undefined) {
			need2ConnectMesssage();
		}
		else if (robotVersion === 2) {
			var pitch = $("#move-head-pitch").val();
			var roll = $("#move-head-roll").val();
			var yaw = $("#move-head-yaw").val();
		}
		else {
			var pitch = $("#move-head-pitch").val();
		}
		...
		showToastMessage("Moving head");
		robo.MoveHead(pitch, roll, yaw, velocity, function (data) {
		console.log(data); });
		});

	$("#set-head-position-form").submit(function (e) {
		e.preventDefault();
		if (robo === undefined) {
			need2ConnectMesssage();
		}
		else {
			var position = $("#set-head-position").val();
			var axis = $("#set-head-axis").val();
			var velocity = 10;

			showToastMessage("Moving head");

			robo.SetHeadToPosition(axis, position, velocity, function (data) {
				console.log(data);
			});
		}
	});

	$("#move-arm-form").submit(function (e) {
		e.preventDefault();
		if (robo === undefined) {
			need2ConnectMesssage();
		}
			...
			var rightArmPosition = $("#set-right-arm-position").val();
			var leftArmPosition = $("#set-left-arm-position").val();

			showToastMessage("Moving arm");

			robo.MoveBothArmsFast(leftArmPosition, rightArmPosition, function (data) {
				console.log(data);
			});
		}
	});
	...
	$("#do-drive-left-turn").on("click", function () {
		var velocity = $("#drive-time-velocity").val();
		if (robotVersion == 2) {
	...
	$("#do-drive-forward").on("click", function () {
		var velocity = $("#drive-time-velocity").val();
		HandleDriveSelection(velocity, 0);
	});

	$("#do-drive-right-turn").on("click", function () {
		var velocity = $("#drive-time-velocity").val();
		if (robotVersion == 2) {
			HandleDriveSelection(velocity, velocity * 0.8);
		}
	...
	$("#do-drive-spin-left").on("click", function () {
		var velocity = $("#drive-time-velocity").val();
		if (robotVersion == 2) {
	...
	$("#do-drive-stop").on("click", function () {
		if (robo === undefined) {
			need2ConnectMesssage();
		}
		else {
	...
	$("#do-drive-spin-right").on("click", function () {
		var velocity = $("#drive-time-velocity").val();
		if (robotVersion == 2) {
			HandleDriveSelection(0, velocity);
		}
	...
	$("#do-drive-backup-left").on("click", function () {
		var velocity = $("#drive-time-velocity").val();
		if (robotVersion == 2) {
			HandleDriveSelection(-velocity, velocity * 0.8);
		}
	...
	$("#do-drive-backward").on("click", function () {
		var velocity = $("#drive-time-velocity").val();
		HandleDriveSelection(-velocity, 0);
	});

	$("#do-drive-backup-right").on("click", function () {
		var velocity = $("#drive-time-velocity").val();
		if (robotVersion == 2) {
			HandleDriveSelection(-velocity, -velocity * 0.8);
		}
	...
	$("#misty-robot-get-battery-level").on("click", function () {
		if (robo === undefined) {
			need2ConnectMesssage();
		}
		else {

			showToastMessage("Getting Battery Level");
			robo.GetBatteryLevel(function (data) { showToastMessage(JSON.stringify(data)) });
		}
	});
	...
	$('#motion-by-value').submit(function (e) {
		e.preventDefault();
		if (robo === undefined) {
			need2ConnectMesssage();
		}
		else {
			var linear = parseFloat($("#linear").val(), 10);
			var angular = parseFloat($("#angular").val(), 10);
			var duration = parseFloat($("#duration-by-value").val(), 10);

			showToastMessage("Driving by Value");
			robo.DriveTimeByValue(linear, angular, duration);
		}
	});

	let motion = undefined;
	$("#motions-dropdown").on("click", function () {
		if (robo === undefined) {
			need2ConnectMesssage();
		}
		else {
			$("#motions").empty();
			$.each(motions, function (key, value) {
				$('#motions').append($('<li class="dropdown-item"></li>').html(key));
			});
		}
	});
	$('#motions').on('click', 'li', function () {
		if (robo === undefined) {
			need2ConnectMesssage();
		}
		else {
			motion = ($(this).text());
			$("#motions-dropdown").text(motion);
			$("#motion-select").removeClass("open");
			$("#motions").empty();
		}
	});

	$('#motion').submit(function (e) {
		e.preventDefault();
		if (robo === undefined) {
			need2ConnectMesssage();
		}
			...
			if (motion) {
				showToastMessage("Driving " + motion + " for " + duration + " milliseconds");
				robo.DriveTime(motion, duration);
			}
			else {
				showToastMessage("Please select a motion first");
			}
